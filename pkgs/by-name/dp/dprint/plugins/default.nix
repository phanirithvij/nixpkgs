...
let
  ...
  iterPlugins =
    cb:
    map (p: "${builtins.toString p}/plugin.wasm") (
      builtins.filter isDerivation (if builtins.isFunction cb then cb plugins else cb)
    );
in
plugins // { inherit mkDprintPlugin iterPlugins; }
